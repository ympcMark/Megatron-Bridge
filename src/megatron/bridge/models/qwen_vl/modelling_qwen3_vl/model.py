...
class Qwen3VLModel(MegatronModule):
    ...
    def _expose_language_model_for_cuda_graph_helper(self) -> None:
        ...
        the model, but in Qwen3-VL these live on ``language_model``. The :meth:`decoder` property already
        exposes the decoder without registering it twice as a child module, which would change distributed
        checkpoint keys from ``language_model.decoder.*`` to ``decoder.*``.
        """
